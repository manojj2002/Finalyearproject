              ...
              <FeatureCard key={index} classes={index<2?'md:col-span-2 lg:col-span-1 xl:col-span-3':'xl:col-span-2'}>
                <>
                  <div className=' p-8 '>
                    ...
                    <motion.div variants={variants.fadeInUp}  className={`w-16 h-16 grid place-items-center rounded-full flex-shrink-0 ${iconBoxColor}`}>{icon}</motion.div>
                    <motion.h3 variants={variants.fadeInUp}  className='text-foreground text-xl font-medium mt-4 mb-3'>{title}</motion.h3>
                    <motion.p className='text-muted-foreground line-clamp-2' variants={variants.fadeInUp} >{desc}</motion.p>
                    <motion.div variants={variants.fadeInUp} >
                      <Button variant='link' className='p-0 h-auto mt-3 '>Learn More <ArrowRight/> </Button>
                    </motion.div>
                  </div>
                </>
                {imgSrc && (
